...
const CourseLandingPage = () => {
  ...
  const courseTitle = courseData.title.split('DIPLOMA IN ')[1] || courseData.title;

  return (
    <>
      <Helmet>
        <title>{courseData.h1} | Shankar Multimedia</title>
        <meta name="description" content={courseData.intro.substring(0, 160)} />
        <meta name="keywords" content={`${courseTitle}, ${courseTitle} course, ${courseTitle} training, Mumbai, 100% job placement`} />
        <link rel="canonical" href={`https://shankarmultimedia.com/course/${courseData.slug}`} />
        
        {/* Open Graph for Social Sharing */}
        <meta property="og:title" content={courseData.h1} />
        ...
        {/* JSON-LD Structured Data - Essential for AI crawlers */}
      ...
      <main className={styles.coursePageContainer} style={{ userSelect: 'text', WebkitUserSelect: 'text' }}>
        ...
        <section className={styles.portfolioSection}>
          <h2 className={styles.sectionHeading}>Student Portfolio Work - {courseTitle}</h2>
        ...
        <section className={styles.finalCtaSection}>
          <h2>Start Your Career in {courseTitle}</h2>
